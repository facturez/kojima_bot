...
- при необходимости передай `DAILY_CHANNEL_ID`
- лучше запускать сервис как worker, а не как публичный web-service

Важно: SQLite-файл внутри контейнера обычно не является постоянным хранилищем. Если хост пересоздаст контейнер, локальная база может пропасть. Для продакшена лучше подключить volume или вынести данные во внешнюю БД.
